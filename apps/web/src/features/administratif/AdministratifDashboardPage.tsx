...
export function AdministratifDashboardPage() {
  ...
  return (
    <div className="w-full space-y-4">
      <div>
        <h2 className="text-base font-black text-ink">{t('tableau_de_bord_administratif')}</h2>
        <p className="mt-0.5 text-xs font-semibold text-ink-soft">{t('suivi_des_dossiers_administrat')}</p>
      </div>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <KpiCard icon={FolderOpen} color="#7B2D9E" bg="#F0DCFA" value={MOCK_DOSSIERS.length} label={t('dossiers')} />
        <KpiCard icon={FileWarning} color="#DC2626" bg="#FEF2F2" value={incomplete} label={t('incomplets')} />
        <KpiCard icon={GraduationCap} color="#22A05E" bg="#D1FAE5" value={MOCK_DOSSIERS.length - incomplete} label={t('complets')} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Link to="/staff/files" className="surface flex items-center gap-3 p-4 transition-colors hover:bg-brand-50">
          ...
          <span className="flex-1 text-sm font-black text-ink">{t('gerer_les_dossiers')}</span>
        ...
        <Link to="/library" className="surface flex items-center gap-3 p-4 transition-colors hover:bg-brand-50">
          ...
          <span className="flex-1 text-sm font-black text-ink">{t('bibliotheque')}</span>
          <ChevronRight size={16} className="text-ink-faint" />
        </Link>
      </div>

      <Card>
        <h3 className="mb-3 text-sm font-black text-ink">{t('dossiers_a_completer_en_priori')}</h3>
