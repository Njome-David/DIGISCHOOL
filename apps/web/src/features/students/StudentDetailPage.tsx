  ...
  return (
    <div className="max-w-4xl space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm font-semibold text-white/80">
              Matricule {student.matricule} - {student.classCode}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <SectionTitle className="mb-4">Informations personnelles</SectionTitle>
          <div className="space-y-3">
            <InfoRow icon={User} label="Sexe" value={student.gender === 'M' ? 'Masculin' : 'Feminin'} />
            <InfoRow
              icon={Cake}
              label="Date de naissance"
              value={`${dateLong(student.dateOfBirth)} (${ageFrom(student.dateOfBirth)} ans)`}
            />
            <InfoRow icon={Flag} label="Nationalite" value={student.nationality} />
            <InfoRow icon={BookOpen} label="Classe" value={student.classCode} />
            <InfoRow icon={CalendarCheck} label="Date d'inscription" value={dateLong(student.enrollmentDate)} />
          </div>
        </Card>

        <Card>
          <SectionTitle className="mb-4">Parent / Tuteur</SectionTitle>
          <div className="space-y-3">
            <InfoRow icon={User} label="Nom" value={student.parentName} />
            <InfoRow icon={Phone} label="Telephone" value={student.parentPhone} />
            <InfoRow icon={Mail} label="Email" value={student.parentEmail || 'Non renseigne'} />
          </div>

          <div className="mt-5 rounded-xl bg-canvas p-3">
            <p className="text-xs font-semibold text-ink-soft">
              Les bulletins, paiements et l'historique scolaire de l'eleve seront accessibles ici une fois les
              modules correspondants actives.
            </p>
          </div>
        </Card>
      </div>
